...
def execute_graz_rnn(model_settings, FLAGS):
    ...
    logits_value, loss_value = sess.run([logits, loss], feed_dict=fd)

    # - Get the weights
    W_in, W_rec, W_out, b_out = sess.run([e for e in tf.compat.v1.trainable_variables()])
    sess.close()

    return_dict = {"logits": logits_value.tolist(), "loss": loss_value.tolist(), "train_input": train_fingerprints.tolist(), "train_groundtruth": train_ground_truth.tolist(), "W_in": W_in.tolist(), "W_rec": W_rec.tolist(), "W_out": W_out.tolist(), "b_out": b_out.tolist()}
...
if __name__ == '__main__':
    ...
    with open("tests/graz_output.json", "w") as f:
