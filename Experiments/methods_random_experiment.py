...
class methods_random_experiment:
    ...
    def train_grid():
        ...
        ecg = [ecg_lsnn.make()]
        ecg0 = configure(ecg, {"beta_robustness": 0.0, "attack_size_mismatch": 0.2})
        ecg1 = configure(ecg, {"beta_robustness": 0.1, "attack_size_mismatch": 0.1, "noisy_forward_std":0.3})
        ecg = ecg0 + ecg1

        speech = [speech_lsnn.make()]
        speech0 = configure(speech, {"beta_robustness": 0.0, "attack_size_mismatch": 0.2})
        speech1 = configure(speech, {"beta_robustness": 0.5, "attack_size_mismatch": 0.1, "noisy_forward_std":0.3})
        speech = speech0 + speech1

        cnn_grid = [cnn.make()]
        cnn_grid0 = configure(cnn_grid, {"beta_robustness": 0.0, "attack_size_mismatch": 0.2})
    ...
    def visualize():
        ...
        # TODO Change attack size mm to 0.1! The results are not right if they are using 0.2
        seeds = [0]
        attack_sizes = [0.0,0.01,0.05,0.1,0.2,0.3,0.5,0.7]
        architecture = "cnn" # ["cnn","speech_lsnn","ecg_lsnn"]
        grid = [model for model in methods_random_experiment.train_grid() if model["seed"] in seeds and model["architecture"]==architecture] 
        ...
        def plot_val(ax, lab, beta, labels=[None,None]):
            ...
            ax.plot(val, color=METHOD_COLORS["Default"], linestyle=METHOD_LINESTYLE["Default"],linewidth=2.0,label=labels[0])
            ...
            ax.set_title(r"$\beta_{\textnormal{rob}}$=" + str(beta))
        ...
        def plot_attack(ax, lab, beta, labels=[None,None]):
            ...
            ax.set_title(r"$\beta_{\textnormal{rob}}$=" + str(beta))
            ax.set_xlabel(r"Attack size $\zeta_{\textnormal{relative}}$")
        ...
        plot_val(axes[0], lab=r"\bf{a}", beta=0.0, labels=[r"$\zeta_{\textnormal{relative}}$=0.0",r"$\zeta_{\textnormal{relative}}>0$"])
        plot_val(axes[1], lab=r"\bf{b}", beta=query([g for g in grid if not g["beta_robustness"]==0.0], "beta_robustness", where={})[0])

        plot_attack(axes[2], lab=r"\bf{c}", beta=0.0, labels=["Attack","Random"])
