...
class mismatch_experiment:
    ...
    def visualize():
        ...
        axes_speech = get_axes_main_figure(fig, gridspecs[0], N_cols, N_rows, "speech", mismatch_levels=speech_mm_levels[1:], btm_ylims=[0.0,0.95])
        axes_ecg = get_axes_main_figure(fig, gridspecs[1], N_cols, N_rows, "ecg", mismatch_levels=ecg_mm_levels[1:], btm_ylims=[0.0,0.95])
        axes_cnn = get_axes_main_figure(fig, gridspecs[2], N_cols, N_rows, "cnn", mismatch_levels=cnn_mm_levels[1:], btm_ylims=[0.0,0.95])

        grid = query(mismatch_experiment.train_grid(), "*", where={"seed":0}) #6 models

        grid = djm(grid, "train", run_mode="load")("{*}")

        grid = split(grid, "mm_level", speech_mm_levels, where={"architecture":"speech_lsnn"}) #16
        grid = split(grid, "mm_level", ecg_mm_levels , where={"architecture":"ecg_lsnn"}) #26
        grid = split(grid, "mm_level", cnn_mm_levels , where={"architecture":"cnn"}) #36

        grid = configure(grid, {"n_iterations":5}) #36
        grid = configure(grid, {"n_iterations":1}, where={"mm_level":0.0}) #36

        grid = configure(grid, {"mode":"direct"}) #36
        
        grid = djm(grid, get_mismatch_list, store_key="mismatch_list")("{n_iterations}", "{*}", "{mm_level}", "{data_dir}")

        def get_data(architecture):
            robust_data = query(grid, "mismatch_list", where={"beta_robustness":1.0, "architecture":architecture})
            vanilla_data = query(grid, "mismatch_list", where={"beta_robustness":0.0, "architecture":architecture})
            return list(zip(vanilla_data, robust_data))
        # - Fill the plots with data

        plot_mm_distributions(axes_speech["btm"], data=get_data("speech_lsnn"))
        plot_mm_distributions(axes_ecg["btm"], data=get_data("ecg_lsnn"))
        plot_mm_distributions(axes_cnn["btm"], data=get_data("cnn"))
