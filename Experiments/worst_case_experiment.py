...
class worst_case_experiment:
    ...
    def train_grid():
        ...
        ecg4 = configure(ecg, {"beta_robustness": 0.0, "noisy_forward":True, "initial_std_mismatch":0.3})
        ...
        speech4 = configure(speech, {"beta_robustness": 0.0, "noisy_forward":True, "initial_std_mismatch":0.3})
        ...
        cnn_grid4 = configure(cnn_grid, {"beta_robustness": 0.0, "noisy_forward":True, "learning_rate":"0.0001,0.00001", "initial_std_mismatch":0.3})
    ...
    def visualize():
        ...
        def get_table(architecture, boundary_loss, loss_or_acc):
            ...
            group_by = ["awp", "beta_robustness", "dropout_prob", "optimizer", "noisy_forward", "attack_size", "n_attack_steps"]
            reduced = reduce_keys(sub_grid, loss_or_acc, reduction={"mean":lambda l : float(np.mean(l))}, group_by=group_by)
            independent_keys = ["n_attack_steps",Table.Deviation_Var({"beta_robustness":0.0, "awp":False, "dropout_prob":0.0, "optimizer":"adam", "noisy_forward":False}, label="Attack"),  "attack_size"]
        ...
        def grid_plot(table, axes_dict):
            ...
            for i0 in range(shape[0]):
                ...
                for i1 in range(shape[1]):
                    ...
                    data_dic = {table.get_label(axis=3, index=idx): [table.get_val(i0,i1,i2,idx,0) for i2 in range(shape[2])] for idx in range(shape[3])}
                    colors = ["#4c84e6","#fc033d","#03fc35","#000000","#9b32a8"]
                    for idx,label in enumerate(data_dic):
                        d = data_dic[label]
                        axes[i1].plot(range(len(d)), d, color=colors[idx], label=label) 
        ...
        def plot(boundary_loss, loss_or_acc):
            ...
            independent_keys = ["architecture","n_attack_steps","attack_size", Table.Deviation_Var({"beta_robustness":0.0, "awp":False, "dropout_prob":0.0, "optimizer":"adam", "noisy_forward":False}, label="Method")]
