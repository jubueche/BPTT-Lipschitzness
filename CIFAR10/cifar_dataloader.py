...
class CIFARDataLoader:
    ...
    def __init__(self, batch_size, data_dir=None):
        ...
        data_path = os.path.join(folder_path, "CIFAR.npy")
        ...
        if os.path.isfile(data_path):
            ...
            train_cifar = tfds.as_numpy(tfds.load("cifar10", data_dir=data_dir, split="train", batch_size=-1))
            test_cifar  = tfds.as_numpy(tfds.load("cifar10", data_dir=data_dir, split="test", batch_size=-1))
            X_train, y_train = train_cifar["image"], train_cifar["label"]
            X_test, y_test = test_cifar["image"], test_cifar["label"]
        ...
        X_train_shaped = np.transpose(X_train_shaped, (0,3,1,2))
    ...
    def get_n_images(self, n_per_class, classes):
        ...
        for c in classes:
            return_dict[str(c)] = []
        found_all = False
        final_batch = np.zeros((n_per_class*len(classes),3,32,32))
