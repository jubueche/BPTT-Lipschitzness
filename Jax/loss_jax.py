...
@partial(jit, static_argnums=(4,5,6,7))
def compute_gradient_and_update2(batch_id, X, y, opt_state, opt_update, get_params, rnn, FLAGS, rand_key):
    params = get_params(opt_state)
    ...
    def training_loss(X, y, params):
        ...
        return loss_normal2(y, logits)

    def lip_loss(X, theta_star, theta, logits):
        logits_theta_star = rnn.call(X, **theta_star)
        return loss_kl(logits, logits_theta_star)
    ...
    def loss_general(X, y, params, FLAGS, rand_key):
        loss_n = training_loss(X, y, params)
        loss_r = robust_loss(X, y, params, FLAGS, rand_key)
        return loss_n + FLAGS.beta_lipschitzness*loss_r
...
@partial(jit, static_argnums=(3,4))
def attack_network2(X, theta, logits, rnn, FLAGS, rand_key):
