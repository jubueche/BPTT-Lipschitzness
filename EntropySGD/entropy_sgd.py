...
def EntropySGD_Jax(step_size,config):
    ...
    def init(w0):
      ...
      state['langevin'] = {'mw': deepcopy(state['wc']),'mdw':deepcopy(state['mdw']),'eta':deepcopy(state['mdw']),'lr':config["langevin_lr"],'beta1':config["langevin_beta1"]}
      return state
    ...
    def update(batch_id, params, state, get_grads):
      ...
      for key in params:
        ...
        state['langevin']['eta'][key] = np.random.normal(loc=0.0, scale=1.0, size=params[key].shape)
      ...
      for i in range(config["L"]):
        ...
        for key in params:
          ...
          if(config["momentum"] > 0):
            state['langevin']['mdw'][key] = config["momentum"] * state['langevin']['mdw'][key] + (1-config["damp"])*grads[key]
            if(config["nesterov"]):
              grads[key] += config["momentum"]*state['langevin']['mdw'][key]
            else:
              grads[key] = state['langevin']['mdw'][key]

          state['langevin']['eta'][key] = np.random.normal(loc=0.0, scale=1.0, size=params[key].shape)
          grads[key] += -g*(state['wc'][key] - params[key]) + config["eps"]/np.sqrt(0.5*config["langevin_lr"])*state['langevin']['eta'][key]

          params[key] += -config["langevin_lr"]*grads[key]
          state['langevin']['mw'][key] = config["langevin_beta1"] * state['langevin']['mw'][key] + (1-config["langevin_beta1"]) * params[key]
      ...
      for key in params:
        ...
        if(config["momentum"] > 0):
          state['mdw'][key] = config["momentum"] * state['mdw'][key] + (1-config["damp"]) * grads[key]
          if(config["nesterov"]):
            grads[key] += config["momentum"] * state['mdw'][key]
          else:
            grads[key] = state['mdw'][key]

        params[key] += -step_size(batch_id)*grads[key]
