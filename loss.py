...
def lipschitzness(theta, theta_start, net, input_batch_t, output_batch_t):
    ...
    # - Compute the loss
    lip = jnp.mean((output_batch_t - spiking_output)**2) / dict_norm_mm(theta_start,theta)
    return lip
...
def dict_norm_mm(d1,d2, eps=1e-6):
    loss = 0
    for key in d1.keys():
        loss += jnp.sqrt(jnp.linalg.norm((d1[key]-d2[key])/(d1[key]+eps))**2 / len(d1[key]))
    return loss / len(d1.keys())
...
def loss_lipschitzness_verbose(
        ...
        for _ in range(number_steps):
            ...
            theta_start_star_distance.append(dict_norm_mm(theta_star,theta_start))
            ...
            for key in theta_star.keys():
                ...
                theta_star[key] = theta_star[key] + step_size*theta_grad[key] / (jnp.linalg.norm(theta_grad[key]) + 1e-4) # Seems to trigger NaNs in gradient when loss is really low
        ...
        loss_dict["random_distance_to_start"] = dict_norm_mm(theta_start, theta_random)
